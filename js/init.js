...
function lexal_tm_menu() {
    ...
    t1.to(".lexal_tm_border.top .trigger .one", 0.5, { y: 6, rotation: 45, ease: Expo.easeinOut });
    t1.to(".lexal_tm_border.top .trigger .two", 0.5, { y: -6, rotation: -45, ease: Expo.easeinOut, delay: -0.5 });
    t1.to(".lexal_tm_menu", 0.5, { right: "-40px", ease: Expo.easeinOut, delay: -0.5 });
        ...
        if (name === "" || email === "" || message === "") {
            ...
            jQuery.post("modal/contact.html", { ajax_name: name, ajax_email: email, ajax_message: message, ajax_subject: subject }, function (data) {
                ...
                if (jQuery(".contact_form .returnmessage span.contact_error").length) {
                    jQuery(".contact_form .returnmessage").slideDown(500).delay(1000).slideUp(500);
                } else {
...
function lexal_tm_anchor() {
    ...
    jQuery(".anchor a").on("click", function (evn) {
        evn.preventDefault();
        jQuery("html,body").scrollTo(this.hash, this.hash, { gap: { y: -scrollOffset - 85 }, animation: { duration: 1500, easing: "easeInOutExpo" } });
        return false;
    });
}
...
function lexal_tm_kenburn_slider() {
    ...
    jQuery(function () {
        jQuery(".lexal_tm_home_hero .overlay_slider").vegas({
            ...
            delay: 1000,
            slides: [{ src: "img/slider/1.jpg" }, { src: "img/slider/2.jpg" }, { src: "img/slider/3.jpg" }],
        });
    });
}
...
function lexal_tm_animate_text() {
    ...
    animateSpan.typed({ strings: ["Freelancer", "UI/UX Designer", "Web Developer"], loop: true, startDelay: 1e3, backDelay: 2e3 });
}
